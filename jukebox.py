...
class DataListBox(Scrollbox):
    ...
    def __init__(self, window, connection, table, field, sort_order=(), **kwargs):
        ...
        self.table = table
        self.field = field  # We call this field as we are using grid manager with tkinter and column
# could be confusing.
    ...
    def link(self, widget, link_field):
        ...
        widget.link_field = link_field  # _id so we know the "master" record we're populated from

    def requery(self, link_value=None):
        self.link_value = link_value  # Store the
        if link_value and self.link_field:
            sql = self.sql_select + " WHERE " + self.link_field + "=?" + self.sql_sort
            print(sql)  # TODO del this line, only for debugging
            self.cursor.execute(sql, (link_value,))
        else:
            print(self.sql_select + self.sql_sort)      # TODO delete this line, only for debugging
    ...
    def on_select(self, event):
        if self.linked_box:
            print(self is event.widget)  # TODO del this line, only for debugging
            if self.curselection():
                index = self.curselection()[0]
                value = self.get(index),

                # Get the id from the database row
                # make sure we are getting the correct one, by including the lnk_value if appropriate
                if self.link_value:
...
if __name__ == '__main__':
    ...
    songsList = DataListBox(mainWindow, conn, "songs", "title", sort_order=("track", "title"))
    # songsList.requery()
